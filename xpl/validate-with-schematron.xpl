...
<p:library xmlns:p="http://www.w3.org/ns/xproc" 
  ...
  <p:declare-step name="validate-with-schematron" type="tr:validate-with-schematron">
    ...
    <p:option name="schematron-rule-msg" select="'no'">
      <p:documentation>Prints a status message with the Id of the currently fired schematron report
        or assert.</p:documentation>
    </p:option>

    <p:input port="source" primary="true" sequence="true">
      <p:documentation>If multiple source documents are given, a report will be created for every
    ...
    <p:input port="parameters" kind="parameter" primary="true">
      <p:documentation>The parameter 'family' is used for looking up and assembling the Schematron
        files from the cascaded configuration.</p:documentation>
    </p:input>
    
    <p:output port="result" primary="true" sequence="true">
      <p:pipe port="result" step="actually-do-something"/>
    </p:output>
    <p:output port="report" sequence="true">
      <p:pipe port="report" step="actually-do-something"/>
    </p:output>
    <p:output port="htmlreport" sequence="true">
      <p:pipe port="htmlreport" step="actually-do-something"/>
    </p:output>
    <p:output port="schema" sequence="true">
      <p:documentation>A compound Schematron document that was assembled by loading the whole
        cascade of Schematrons for the given family and merging it so that only the most specific
        pattern for a given pattern ID wins.</p:documentation>
      ...
      <p:when test="$active = ('true', 'yes')">
        ...
        <p:string-replace match="c:family" name="replace-schematron-family-name-in-start-msg">
          <p:with-option name="replace"
            select="concat('''', /c:param-set/c:param[@name eq 'family']/@value, '''')">
            <p:pipe port="result" step="consolidate-params"/>
          </p:with-option>
          <p:input port="source">
            <p:inline>
              <c:messages>
                <c:message xml:lang="en">Starting '<c:family>family</c:family>' Schematron
                  checks</c:message>
                <c:message xml:lang="de">Beginne Schematron-Prüfungen der Rubrik
                    '<c:family>family</c:family>'</c:message>
              </c:messages>
            </p:inline>
          </p:input>
        </p:string-replace>
        ...
        <tr:simple-progress-msg name="start-msg">
          <p:with-option name="file"
            select="concat('validate-with-schematron_', /c:param-set/c:param[@name eq 'family']/@value,'_start.txt')">
            <p:pipe port="result" step="consolidate-params"/>
          </p:with-option>
          <p:input port="msgs">
        ...
        <p:choose name="validate-if-assembled-has-pattern">
          ...
          <p:when test="//s:pattern">
            <p:output port="report" sequence="true">
              <p:pipe step="pattern-in-assembled" port="report"/>
            </p:output>
            <p:output port="result" primary="true" sequence="true">
              <p:pipe step="pattern-in-assembled" port="result"/>
            </p:output>
            <p:output port="htmlreport" sequence="true">
              <p:pipe step="pattern-in-assembled" port="htmlreport"/>
            </p:output>
            <tr:validate-with-schematron2 name="pattern-in-assembled">
              ...
              <p:input port="html-in">
                <p:pipe port="html-in" step="validate-with-schematron"/>
              </p:input>
              <p:input port="schema">
                <p:pipe port="result" step="assemble-schematron"/>
              </p:input>
          ...
          <p:otherwise>
            ...
            <p:output port="htmlreport" sequence="true">
              <p:pipe step="validate-with-schematron" port="html-in"/>
            </p:output>
            <p:identity name="id">
              <p:input port="source">
                <p:pipe port="source" step="validate-with-schematron"/>
        ...
        <p:string-replace match="c:family" name="replace-schematron-family-name-in-success-msg">
          ...
          <p:input port="source">
            <p:inline>
              <c:messages>
                <c:message xml:lang="en">Successfully finished '<c:family>family</c:family>'
                  Schematron checks</c:message>
                <c:message xml:lang="de">Schematron-Prüfungen der Rubrik
                    '<c:family>family</c:family>' erfolgreich abgeschlossen</c:message>
              </c:messages>
            </p:inline>
          </p:input>
        </p:string-replace>
        ...
        <tr:simple-progress-msg name="success-msg">
          <p:with-option name="file"
            select="concat('validate-with-schematron_', /c:param-set/c:param[@name eq 'family']/@value,'_success.txt')">
            <p:pipe port="result" step="consolidate-params"/>
          </p:with-option>
          <p:input port="msgs">
  ...
  <p:declare-step type="tr:validate-with-schematron2" name="validate-with-schematron2">
    ...
    <p:for-each name="validate-loop">
      ...
      <p:add-attribute match="/*" attribute-name="tr:step-name">
        <p:with-option name="attribute-value"
          select="/c:param-set/c:param[@name eq 'step-name']/@value">
          <p:pipe port="result" step="consolidate-params"/>
        </p:with-option>
        <p:input port="source">
      ...
      <p:add-attribute match="/*" attribute-name="tr:rule-family" name="add-family-attribute">
        <p:with-option name="attribute-value"
          select="/c:param-set/c:param[@name eq 'family']/@value">
          <p:pipe port="result" step="consolidate-params"/>
        </p:with-option>
      </p:add-attribute>
    ...
    <p:for-each name="conditionally-patch-html">
      <p:documentation>This is no actual loop. It is just for patching the HTML input if there is an
        HTML document on the html-in port, and to do nothing if there isn't.</p:documentation>
      <p:iteration-source>
      ...
      <cx:message cx:depends-on="conditionally-patch-html">
        <p:input port="source">
          <p:pipe step="conditionally-patch-html" port="current"/>
        </p:input>
        <p:with-option name="message" select="'conditionally-patch-html: ', /*/name()"/>
      </cx:message>
